	...
	*/
	...
	var resizePizzas = function(size) {
	  ...
	  function changePizzaSizes(size) {
	    'use strict';
	    var mutantPizzas = document.getElementsByClassName("randomPizzaContainer");
	    var dx = determineDx(mutantPizzas[0], size);
	    var newwidth = (mutantPizzas[0].offsetWidth + dx) + 'px';
	    for (var i = 0, y = mutantPizzas.length; i < y; i++) {
	      mutantPizzas[i].style.width = newwidth;
	    }
	  }
	...
	var pizzasDiv = document.getElementById("randomPizzas"); // moved variable outside of loop so the DOM will only be called once
	for (var i = 2; i < 100; i++) {
